      ...
      {roles.map((role) => (
        ...
        >
          <div className="absolute top-4 right-4 flex items-center justify-center">
            <div
              className={`flex h-5 w-5 items-center justify-center rounded-full border-2 transition-all duration-300 ${selectedRole === role.id ? 'border-brand-purple-600' : 'border-slate-300'} `}
            >
              {selectedRole === role.id && (
          ...
          <div className="mb-8 flex h-24 items-center justify-center rounded-xl border border-slate-100 bg-slate-50/70 transition-transform duration-300">
            {role.icon}
          </div>
          <div className="flex-1">
            <h3 className="mb-3 text-xl font-bold text-slate-900">{role.title}</h3>
            <p className="text-base leading-relaxed text-slate-500">{role.desc}</p>
          </div>
