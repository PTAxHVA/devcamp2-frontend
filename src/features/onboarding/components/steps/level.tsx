    ...
    <div className="mb-20 grid w-full grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
      {levels.map((level) => (
        <div
          ...
          className={`bg-bg-card relative flex min-h-70 cursor-pointer flex-col items-center justify-center rounded-2xl p-8 text-center transition-all duration-300 ${
        ...
        >
          <div className="absolute top-4 right-4 flex items-center justify-center">
            <div
              className={`flex h-5 w-5 items-center justify-center rounded-full border-2 transition-all duration-300 ${selectedLevel === level.id ? 'border-brand-purple-600' : 'border-border-input'} `}
            >
              {selectedLevel === level.id && (
