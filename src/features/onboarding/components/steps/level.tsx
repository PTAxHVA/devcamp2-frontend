      ...
      {levels.map((level) => (
        ...
        >
          <div className="absolute top-4 right-4 flex items-center justify-center">
            <div
              className={`flex h-5 w-5 items-center justify-center rounded-full border-2 transition-all duration-300 ${selectedLevel === level.id ? 'border-brand-purple-600' : 'border-slate-300'} `}
            >
              {selectedLevel === level.id && (
          ...
          <div className="mt-4 mb-6 flex items-center justify-center transition-transform duration-300">
            <div className="bg-brand-purple-50 rounded-2xl p-4">{level.icon}</div>
          </div>

          <h3 className="mb-2 text-lg font-bold text-slate-900">{level.title}</h3>
          <p className="text-sm leading-relaxed text-slate-500">{level.desc}</p>
        </div>
      ))}
    </div>
  )
